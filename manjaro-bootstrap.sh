...
PACMAN_PACKAGES=(
  ...
  libassuan libgpg-error libnghttp2 libidn2 libssh2 libunistring lzo openssl pacman pacman-mirrors xz zlib zstd
...
install_packages() {
  ...
  LC_ALL=C chroot "$DEST" /usr/bin/pacman \
    --noconfirm --arch $ARCH -Sy --overwrite /bin*,/lib*,/usr/*,/etc/*,/sbin*,/var/* $PACKAGES
}
